...
module.exports = function(wave, note, options) {
  ...
  // unused variable
  // var samplingFreq = freq * N163.constants.SAMPLES_PER_FRAMES
  
  var n = N163.constants.SAMPLES_PER_FRAMES * N163.constants.FRAMES_PER_TABLE
  var data = wave.channelData[options.channel]
  
  // check for interpolation
  if (options.linear || options.exp) {
    var cycles = ~~((data.length * freq) / wave.sampleRate)
    var cycleInterval = cycles/N163.constants.FRAMES_PER_TABLE
    console.log('Samples: ' + data.length + '\nSample rate: ' + wave.sampleRate + '\nCycles: ' + cycles)
    var rangeList = []
  
    // linear interpolation
    if (options.linear || cycles < N163.constants.SAMPLES_PER_FRAMES) {
      if (options.exp) {
      ...
      sample = 0
      for (cycle = 0; cycle < N163.constants.FRAMES_PER_TABLE; cycle++) {
        sample = ~~(cycleInterval) * cycle * N163.constants.SAMPLES_PER_FRAMES
        for (nextSample=0; nextSample < N163.constants.SAMPLES_PER_FRAMES; nextSample++) {
          // console.log(sample)
  ...
  // "exponential" interpolation, sort of
      ...
      sample = 0
      cycleLog = Math.pow(Math.E, (Math.log(cycles) / N163.constants.FRAMES_PER_TABLE))
      for (cycle = 0; cycle < N163.constants.FRAMES_PER_TABLE; cycle++) {
        // console.log('Cycle number ' + (cycle/N163.constants.SAMPLES_PER_FRAMES))
        for (nextSample=0; nextSample < N163.constants.SAMPLES_PER_FRAMES; nextSample++) {
          // console.log(cycle)
          rangeList.push(~~(sample))
          sample = sample + 1
        }
        sample = (Math.round(Math.pow(cycleLog,cycle*1.1)) * N163.constants.SAMPLES_PER_FRAMES) + (cycle * N163.constants.SAMPLES_PER_FRAMES)
      }
    }
  ...
  var samples = _.map(rangeList, function (i) {
   // compute adequate time
    var t = (i / (freq * N163.constants.SAMPLES_PER_FRAMES))
  ...
  samples = _.map(samples, function(float) {
    ...
    return ~~((float + 1.0) * N163.constants.FRAME_RESOLUTION * 0.5)
	  }
  })
